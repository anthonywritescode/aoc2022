...
def compute(s: str) -> int:
    ...
    seen = set()
    todo = collections.deque([(0, start_x, start_y, up, down, left, right)])
    while todo:
        depth, x, y, up, down, left, right = todo.popleft()
        ...
        # wait
        todo.append((depth + 1, x, y, *next_masks))

        # move
        for (cx, cy) in support.adjacent_4(x, y):
            if 0 <= cx < width and 0 <= cy < height:
                todo.append((depth + 1, cx, cy, *next_masks))
