...
AS (
    ...
    SELECT
        nn.i + 1,
        nn.n * (SELECT monk.mod FROM monk WHERE monk.id = (nn.i + 1))
    FROM nn WHERE nn.i <= (SELECT MAX(monk.id) FROM monk)
)
INSERT INTO modulus
SELECT MAX(n) FROM nn;
...
CREATE TRIGGER t_work AFTER INSERT ON work FOR EACH ROW BEGIN
    ...
    UPDATE items
    SET val = val % (SELECT n FROM modulus)
    WHERE monk = NEW.monk;
...
INSERT INTO work
SELECT monk FROM nn WHERE i < 10000;
